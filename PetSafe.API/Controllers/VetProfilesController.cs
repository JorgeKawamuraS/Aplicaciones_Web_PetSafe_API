...
namespace PetSafe.API.Controllers
{
    ...
    public class VetProfilesController : ControllerBase
    {
        private readonly IVetProfileService _vetProfileService;
        private readonly IMapper _mapper;

        public VetProfilesController(IVetProfileService vetProfileService, IMapper mapper)
        {
        ...
        [ProducesResponseType(typeof(VetProfileResource), 200)]
        [ProducesResponseType(typeof(BadRequestResult), 404)]
        public async Task<IActionResult> PostAsync([FromBody] SaveVetProfileResource resource)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState.GetErrorMessages());

            var vetProfile = _mapper.Map<SaveVetProfileResource, VetProfile>(resource);
            var result = await _vetProfileService.SaveAsync(vetProfile);
