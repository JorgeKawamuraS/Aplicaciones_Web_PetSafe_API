...
namespace PetSafe.API.Controllers
{
    ...
    public class OwnerProfilesController : ControllerBase
    {
        ...
        public OwnerProfilesController(IOwnerProfileService ownerProfileService, IMapper mapper)
        {
        ...
        [ProducesResponseType(typeof(OwnerProfileResource), 200)]
        [ProducesResponseType(typeof(BadRequestResult), 404)]
        public async Task<IActionResult> PostAsync([FromBody] SaveOwnerProfileResource resource)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState.GetErrorMessages());

            var ownerProfile = _mapper.Map<SaveOwnerProfileResource, OwnerProfile>(resource);
            var result = await _ownerProfileService.SaveAsync(ownerProfile);
