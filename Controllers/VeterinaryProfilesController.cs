...
namespace PetSafe.API.Controllers
{
    ...
    public class VeterinaryProfilesController : ControllerBase
    {
        private readonly IVeterinaryProfileService _veterinaryProfileService;
        private readonly IMapper _mapper;

        public VeterinaryProfilesController(IVeterinaryProfileService veterinaryProfileService, IMapper mapper)
        {
        ...
        [ProducesResponseType(typeof(VeterinaryProfileResource), 200)]
        [ProducesResponseType(typeof(BadRequestResult), 404)]
        public async Task<IActionResult> PostAsync([FromBody] SaveVeterinaryProfileResource resource)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState.GetErrorMessages());

            var veterinaryProfile = _mapper.Map<SaveVeterinaryProfileResource, VeterinaryProfile>(resource);
            var result = await _veterinaryProfileService.SaveAsync(veterinaryProfile);
