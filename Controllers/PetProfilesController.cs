...
namespace PetSafe.API.Controllers
{
    ...
    public class PetProfilesController : ControllerBase
    {
        ...
        public PetProfilesController(IPetProfileService petProfileService, IMapper mapper)
        {
        ...
        [ProducesResponseType(typeof(PetProfileResource), 200)]
        [ProducesResponseType(typeof(BadRequestResult), 404)]
        public async Task<IActionResult> PostAsync([FromBody] SavePetProfileResource resource)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState.GetErrorMessages());

            var petProfile = _mapper.Map<SavePetProfileResource, PetProfile>(resource);
            var result = await _petProfileService.SaveAsync(petProfile);
